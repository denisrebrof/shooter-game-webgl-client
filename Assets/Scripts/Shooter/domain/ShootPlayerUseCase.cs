...
namespace Shooter.domain
{
    public class ShootPlayerUseCase : IDisposable
    {
        ...
        public void Hit(long receiverId, long weaponId, int damage, Vector3 position)
        {
        ...
        private void RequestShoot(ShootRequestData data) =>
            commandsUseCase
                .Request<Unit, ShootRequestData>(Commands.IntentShoot, data)
                .Subscribe()
                .AddTo(composite);

        private void RequestHit(HitRequestData data) =>
            commandsUseCase
