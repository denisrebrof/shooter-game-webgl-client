...
namespace Shooter.presentation.Player.IK
{
    ...
    public class CustomIK : MonoBehaviour
    {
        ...
        private void CalculateIK()
        {
            ...
            Vector3 elbowHeight = Vector3.Cross((targetIK.position - rootBone.position).normalized,
                (elbowIK.position - elbowsAdjacent).normalized) * -1;
            Vector3 jointPosition = adjacent +
                                    Vector3.Cross((targetIK.position - rootBone.position).normalized, elbowHeight) *
                                    triangleHeight;
        ...
        Vector3 CalculateElbowAdjacent()
        {
            ...
            int isObtuse = 1;
            if (Mathf.Pow(upperArmToElbowLength, 2f) + Mathf.Pow(upperArmToTargetLength, 2f) <
                Mathf.Pow(elbowToTargetLength, 2f))
            {
                isObtuse = -1;
            }

            Vector3 adjacent = (targetIK.position - rootBone.position).normalized * adjacentLength * isObtuse +
                               rootBone.position;
        ...
        float CalculateTriangleHeight(float side1, float side2, float side3)
        {
            ...
            float triangleArea = Mathf.Sqrt(halfPerimeter * (halfPerimeter - side1) * (halfPerimeter - side2) *
                                            (halfPerimeter - side3));
