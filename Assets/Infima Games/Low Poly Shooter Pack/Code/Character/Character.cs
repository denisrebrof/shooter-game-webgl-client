...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public sealed class Character : CharacterBehaviour
    {
        ...
        [Tooltip("The character's kinematics")] [SerializeField]
        ...
        private Coroutine reloadIKCoroutine;
        ...
        private void OnDisable() => StopReloadingIK();
        ...
        private void UpdateAnimator()
        {
            ...
            if (characterAnimator.GetBool(boolNameReloading))
            {
                ...
                if (equippedWeapon.GetAmmunitionTotal() - equippedWeapon.GetAmmunitionCurrent() < 1)
                {
                    ...
                    var animator = equippedWeapon.GetAnimator();
                    if (animator != null) animator.SetBool(boolNameReloading, false);
                }
            }
        ...
        private void PlayReloadAnimation()
        {
            ...
            string stateName = equippedWeapon.HasCycledReload()
                ? "Reload Open"
                : (!equippedWeapon.HasEmptyReload() || equippedWeapon.HasAmmunition() ? "Reload" : "Reload Empty");
            ...
            StopReloadingIK();
            reloadIKCoroutine = StartCoroutine(ReloadIKCoroutine());
        }

        private void StopReloadingIK()
        {
            if(reloadIKCoroutine!=null) StopCoroutine(reloadIKCoroutine);
            ResetIKOffsets();
            reloading = false;
        }

        private IEnumerator ReloadIKCoroutine()
        {
            var data = equippedWeapon.GetReloadIKData();
            if(data.skipIK)
                yield break;
            
            yield return new WaitForSeconds(data.inIKDelay);
            
            var timer = data.inIKDur;
            var defaultOffset = equippedWeapon.GetLeftHandOffset();
            while (timer > 0)
            {
                timer -= Time.deltaTime;
                var progress = 1f - timer / data.inIKDur;
                var ikPos = Vector3.Lerp(defaultOffset, data.offset, progress);
                kinematics.UpdateLeftEffector(ikPos);
                yield return null;
            }
            kinematics.UpdateLeftEffector(data.offset);
            yield return new WaitForSeconds(data.outIKDelay);
            ...
            while (timer > 0)
            {
                timer -= Time.deltaTime;
                var progress = 1f - timer / data.inIKDur;
                var ikPos = Vector3.Lerp(data.offset, defaultOffset, progress);
                kinematics.UpdateLeftEffector(ikPos);
                yield return null;
            }
            ResetIKOffsets();
        ...
        private void ResetIKOffsets()
        {
            var leftOffset = equippedWeapon.GetLeftHandOffset();
            var rightOffset = equippedWeapon.GetRightHandOffset();
            kinematics.UpdateEffectors(leftOffset, rightOffset);
        }
        ...
        {
            Debug.Log("CanPlayAnimationReload 0");
            //No reloading!
            if (reloading)
                return false;
            Debug.Log("CanPlayAnimationReload 1");
            //No meleeing!
            if (meleeing)
                return false;
            Debug.Log("CanPlayAnimationReload 2");
            //Not actioning a bolt.
            if (bolting)
                return false;
            Debug.Log("CanPlayAnimationReload 3");
            //Can't reload while throwing a grenade.
            if (throwingGrenade)
                return false;
            Debug.Log("CanPlayAnimationReload 4");
            //Block while inspecting.
            if (inspecting)
                return false;
            Debug.Log("CanPlayAnimationReload 5");
            //Block Full Reloading if needed.
            if (!equippedWeapon.CanReloadWhenFull() && equippedWeapon.IsFull())
                return false;
            Debug.Log("CanPlayAnimationReload 6");
            //Return.
            return true;
        }
        ...
        {
            Debug.Log("OnTryFire 1");
            //Block while the cursor is unlocked.
            if (!cursorLocked)
                return;
            Debug.Log("OnTryFire 2");
            ...
            switch (context)
            {
                ...
                case { phase: InputActionPhase.Performed }:
                    ...
                    Debug.Log("OnTryFire 4");
                    if (!CanPlayAnimationFire())
                        break;

                    Debug.Log("OnTryFire 5");
                    //Check.
                    if (equippedWeapon.HasAmmunition())
                    {
                        ...
                        Debug.Log("OnTryFire 7");
                    }
                    //Fire Empty.
                    else
                    {
                        FireEmpty();
                        Debug.Log("OnTryFire 8");
                    }
        ...
        {
            Debug.Log("OnTryPlayReload 1");
            //Block while the cursor is unlocked.
            if (!cursorLocked)
                return;

            Debug.Log("OnTryPlayReload 2");
            //Block.
            if (!CanPlayAnimationReload())
                return;

            Debug.Log("OnTryPlayReload 3");
            //Switch.
            switch (context)
            {
                //Performed.
                case { phase: InputActionPhase.Performed }:
                    //Play Animation.
                    PlayReloadAnimation();
                    Debug.Log("OnTryPlayReload 4");
                    break;
            }
            
            Debug.Log("OnTryPlayReload 5");
        }
